...
int nn_loop(int argc, char **argv) {
  ...
  int num_neurons_per_hidden_layer = 20;
  int num_outputs = 6;
  nn_ctl_ = genann_init(num_inputs, num_hidden_layers, num_neurons_per_hidden_layer, num_outputs);
  
  bool new_input_data = false;
    ...
    if (poll_ret == 0) {
      ...
      if (fds[0].revents & POLLIN) { // input_rc
        ...
        if (nn_ctl_in.type == nn_in_s::NN_CTL_TRAIN) {
          ...
          retrieve_new_input(new_input, &new_input);

          if (old_input_data && &new_input != &old_input) {

            label = old_input[:6]
            new_input = preprocess_inputs((const double *)nn_ctl_in.data);
            genann_train(
              nn_ctl_,
              new_input, //(const double *)nn_ctl_in.data,
              label, //(const double *)nn_ctl_in.data,
              .01);
